...
public class MyCommandLineRunner implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        ...
        for (String aClassName: Constant.METHOD_URL_SET) {
            ...
            String baseUrl = "";
            String[] classUrl ={};
            if(!CommonUtil.isEmpty(clazz.getAnnotation(RequestMapping.class))){
            ...
            for (Method method:methods) {
                if(method.isAnnotationPresent(Pass.class)){
                    ...
                    if(!CommonUtil.isEmpty(method.getAnnotation(PostMapping.class))){
                        methodUrl=method.getAnnotation(PostMapping.class).value();
                        if(CommonUtil.isEmpty(methodUrl)){
                            methodUrl=method.getAnnotation(PostMapping.class).path();
                        }
                        baseUrl=getRequestUrl(classUrl, methodUrl, sb,"POST");
                    }else if(!CommonUtil.isEmpty(method.getAnnotation(GetMapping.class))){
                    ...
                    }else if(!CommonUtil.isEmpty(method.getAnnotation(DeleteMapping.class))){
                    ...
                    }else if(!CommonUtil.isEmpty(method.getAnnotation(PutMapping.class))){
    ...
    private String  getRequestUrl(String[] classUrl, String[] methodUrl, StringBuilder sb,String requestName) {
        sb.append("/api/v1");
        if(!CommonUtil.isEmpty(classUrl)){
